...
exports.createSimpleFlow = createSimpleFlow;
exports.createMillwareAttendanceFlow = createMillwareAttendanceFlow;
...
function createMillwareAttendanceFlow() {
    return {
...
async function runAutomation() {
    // Create logger
    const logger = new index_1.Logger('Main');
    // Create event bus
    ...
    const actionRegistry = new index_1.ActionRegistry();
    // Register all navigation actions
    for (const ActionClass of index_1.navigationActions) {
        actionRegistry.register(ActionClass);
    }
    // Register custom action
    // actionRegistry.register(CustomAction);
    // Create provider
    const provider = new index_1.MillwareProvider({
    ...
    const browserAdapter = new index_1.PuppeteerAdapter({
        headless: false,
        slowMo: 0,
    });
    // Create agent engine
    const engine = new index_1.AgentEngine({
        provider,
        browserAdapter,
        actionRegistry,
        eventBus,
