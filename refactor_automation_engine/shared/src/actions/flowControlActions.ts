...
import { ExecutionContext, ActionParams, ActionResult } from './baseAction';

/**
 * ForEach Action - Iterate over an array
 */
export class ForEachAction extends AbstractAction {
  constructor() {
    super('forEach');
  }
  ...
  async execute(
    ...
    const { items, itemName, indexName, parallel, maxConcurrency } = params as {
      items: string;
      itemName: string;
      indexName?: string;
      parallel?: boolean;
    ...
    const results: Array<{ item: unknown; index: number; result: unknown }> = [];
  ...
  private getValueFromContext(path: string, context: ExecutionContext): unknown {
    ...
    let value: unknown = context.data || context.variables;
 ...
 */
export class IfAction extends AbstractAction {
  constructor() {
    super('if');
  }
 ...
 */
export class SwitchAction extends AbstractAction {
  constructor() {
    super('switch');
  }
  ...
  async execute(
    ...
    const { value, cases, default: defaultCase } = params as {
      value: string;
 ...
 */
export class ParallelAction extends AbstractAction {
  constructor() {
    super('parallel');
  }
  ...
  async execute(
    ...
    const { branches, failFast, maxConcurrency } = params as {
 ...
 */
export class TryAction extends AbstractAction {
  constructor() {
    super('try');
  }
  ...
  async execute(
    ...
    const { maxRetries, retryDelay, backoffMultiplier } = params as {
      maxRetries?: number;
      retryDelay?: number;
 ...
 */
export class WhileAction extends AbstractAction {
  constructor() {
    super('while');
  }
  ...
  async execute(
    ...
    const { condition, maxIterations } = params as {
      condition: string;
 ...
 */
export class WaitForNavigationAction extends AbstractAction {
  constructor() {
  ...
  async execute(
    ...
    const { waitUntil, timeout } = params as {
