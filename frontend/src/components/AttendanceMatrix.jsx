...
const AttendanceMatrix = ({
    ...
    const getSyncStatus = (ptrjId, dateStr, venusStatus, venusRegularHours = 0, venusOtHours = 0) => {
        ...
        if (!hasValidPtrjId) return { status: 'not_synced', displayOverride: 'N/A', displayColor: '#DE350B', borderWidth: 2, isUnmapped: true };
        ...
        if (compareMode === 'presence') {
            if (!millwareRecord || !millwareRecord.hasRegularRecord) return { status: 'not_synced', displayOverride: `${venusRegularHours}h`, displayColor: '#DE350B', borderWidth: 2 };
        ...
        if (compareMode === 'overtime') {
            ...
            if (!millwareRecord || !millwareRecord.hasOTRecord) return { status: 'not_synced', displayOverride: `${vOT}h`, displayColor: '#DE350B', borderWidth: 2 };
            if (!millwareRecord.otMatched) return { status: 'mismatch', displayOverride: `${vOT}h|${millwareRecord.ot}h`, displayColor: '#FF991F', borderWidth: 2, millwareHours: millwareRecord.ot || 0 };
                        ...
                        {safeData.map((emp) => {
                            ...
                            return (
                                <React.Fragment key={emp.id}>
                                    <TableRow hover selected={selectedIds.includes(emp.id)} sx={{ height: 32, bgcolor: editingRow === emp.id ? '#FFF9C4' : 'inherit' }}>
                                        <TableCell sx={{ position: 'sticky', left: 0, zIndex: 101, bgcolor: 'inherit', p: 0 }} align="center">
                                            <IconButton size="small" onClick={() => toggleRow(emp.id)} sx={{ p: 0.2 }}>{expandedRows.has(emp.id) ? <ExpandMoreIcon sx={{ fontSize: 16 }} /> : <ChevronRightIcon sx={{ fontSize: 16 }} />}</IconButton>
                                        </TableCell>
                                        <TableCell sx={{ position: 'sticky', left: 32, zIndex: 101, bgcolor: 'inherit', borderRight: '2px solid #F0F0F0 !important' }}>
                                        ...
                                        <TableCell sx={{ position: 'sticky', left: 212, zIndex: 101, bgcolor: editingRow === emp.id ? '#FFF8E1' : 'inherit', borderRight: '2px solid #F0F0F0 !important', cursor: 'pointer' }} onClick={() => handleStartEdit(emp)}>
