            ...
            <TableRow hover onClick={handleToggle} sx={{ cursor: 'pointer', bgcolor: open ? '#f0fdf4' : 'inherit' }}>
                ...
                <TableCell align="center">
                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5 }}>
                        ...
                        <Typography sx={{ fontSize: '0.6rem', fontWeight: 600, color: '#d97706' }}>{stats.totalJamOTRange.toFixed(2)}h</Typography>
                    </Box>
                </TableCell>
                <TableCell align="center">
                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5 }}>
                        <Typography sx={{ fontWeight: 800, color: '#7c3aed' }}>{stats.totalJamOTRange.toFixed(2)}h</Typography>
                        <Typography sx={{ fontSize: '0.6rem', fontWeight: 600, color: '#a78bfa' }}>avg: {(employees.length > 0 ? (stats.totalJamOTRange / employees.length).toFixed(2) : 0)}h/org</Typography>
                    </Box>
                </TableCell>
            </TableRow>
            <TableRow>
                            ...
                            <Table size="small" sx={{ bgcolor: 'white', border: '1px solid #e2e8f0', borderRadius: 1 }}>
                                <TableHead>
                                    <TableRow sx={{ bgcolor: '#f8fafc' }}>
                                        ...
                                        <TableCell align="center" sx={{ fontWeight: 800, fontSize: '0.7rem', color: '#7c3aed' }}>JAM SPL DLM RANGE</TableCell>
                                    </TableRow>
                                </TableHead>
                                <TableBody>
                                    {employees.map((emp) => (
                                        <TableRow key={emp.id} hover>
                                            ...
                                            <TableCell align="center" sx={{ fontSize: '0.8rem', fontWeight: 800, color: '#7c3aed' }}>{emp.totalJamOTRange.toFixed(2)}h</TableCell>
                                        </TableRow>
                                    ))}
                                    <TableRow sx={{ bgcolor: '#F1F5F9', fontWeight: 800 }}>
                                        ...
                                        <TableCell align="center" sx={{ fontWeight: 800, fontSize: '0.8rem', color: '#7c3aed' }}>{stats.totalJamOTRange.toFixed(2)}h</TableCell>
                                    </TableRow>
                                </TableBody>
                            </Table>
                        </Box>
...
const OvertimeRangeReport = ({
    ...
    const handleExportPDF = () => {
        ...
        doc.setFontSize(18);
        doc.text('LAPORAN RANGE SPL (SURAT PERINTAH LEMBUR)', 105, 15, { align: 'center' });
        doc.setFontSize(11);
        ...
        filteredStationData.forEach(st => {
            ...
            y += 5;
            const rows = st.employees.map((e, i) => [i+1, e.name, e.id, e.hariDalamRange, `${e.totalJamOTRange.toFixed(2)}h`]);
            autoTable(doc, {
                head: [['No', 'Nama', 'ID', 'Hari dlm Range', 'Total Jam SPL Range']],
                body: rows,
                startY: y,
                theme: 'grid',
    ...
    return (
        <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', p: 2 }}>
            <Box sx={{ mb: 3 }}>
                <Typography variant="h5" sx={{ fontWeight: 900, color: '#1e293b', mb: 1 }}>REPORT RANGE SPL</Typography>
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={4}>
                        <Paper sx={{ p: 2, borderLeft: '4px solid #7c3aed', bgcolor: '#f5f3ff' }}>
                            <Typography variant="caption" sx={{ fontWeight: 800, color: '#6d28d9' }}>TOTAL JAM SPL DALAM RANGE</Typography>
                            <Typography variant="h4" sx={{ fontWeight: 900 }}>{totalStats.grandTotalJamOTRange.toFixed(2)}h</Typography>
                        </Paper>
                    </Grid>
                </Grid>
            </Box>
                ...
                <Table stickyHeader size="small">
                    <TableHead>
                        <TableRow>
                            ...
                            <TableCell align="center" sx={{ bgcolor: '#1e293b', color: 'white', fontWeight: 800 }}>
                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                    <Typography sx={{ fontSize: '0.7rem' }}>TOTAL JAM SPL RANGE</Typography>
                        ...
                        <TableRow sx={{ bgcolor: '#1e293b', fontWeight: 800 }}>
                            ...
                            <TableCell align="center">
                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5 }}>
                                    ...
                                    <Typography sx={{ fontSize: '0.65rem', fontWeight: 700, color: '#fcd34d' }}>{totalStats.grandTotalJamOTRange.toFixed(2)}h</Typography>
                                </Box>
                            </TableCell>
                            <TableCell align="center">
                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5 }}>
                                    <Typography sx={{ fontWeight: 800, color: '#fbbf24', fontSize: '0.85rem' }}>{totalStats.grandTotalJamOTRange.toFixed(2)}h</Typography>
                                    <Typography sx={{ fontSize: '0.65rem', fontWeight: 700, color: '#fcd34d' }}>avg: {(totalStats.totalKaryawan > 0 ? (totalStats.grandTotalJamOTRange / totalStats.totalKaryawan).toFixed(2) : 0)}h/org</Typography>
                                </Box>
                            </TableCell>
                        </TableRow>
                    </TableBody>
